...
const WorkoutCard: React.FC<WorkoutCardProps> = ({ workout, selectedDate, selectedTime }) => {
  ...
  return (
    <>
      <article
        className={`border rounded-2xl p-4 shadow-md flex flex-col gap-4 w-full ${
          isDarkMode ? "border-gray-700 bg-gray-800 text-white" : "border-gray-100 bg-white text-black"
        }`}
      >
          ...
          <div className="flex items-start gap-4">
            <img
              ...
              onError={(e) => {
                ...
                target.src = "/images/default-coach.jpg";
              }}
            />
            <div>
              ...
              <div className="mt-1">
                <span className="text-sm text-yellow-600 font-medium">{workout.rating}⭐</span>
              </div>
            </div>
          </div>

          <div
            className={`relative border rounded-lg p-4 flex-1 min-w-[150px] ${
              isDarkMode ? "border-lime-500 bg-gray-700 text-white" : "border-lime-400 bg-white text-gray-700"
            }`}
          >
            <h3
              className={`absolute -top-3 left-3 px-2 text-sm font-semibold ${
            ...
            <ul className="text-sm mt-2 space-y-2">
              ...
              <li className="flex items-center gap-2">
                <Clock4 size={16} />
                <strong>Time:</strong> 1 hr
              </li>
              <li className="flex items-center gap-2">
                <CalendarFold size={16} />
                <strong>Date:</strong>{" "}
                {`${selectedDate.toLocaleDateString("en-US", { month: "long", day: "numeric" })}, ${convertTo12HourFormat(
                  workout.selectedTime
                )}`}
              </li>
            </ul>
          </div>
        ...
        <p className="text-sm">{description[workout.type as WorkoutType]}</p>

        <div>
          <h3 className="text-sm font-semibold">Also available for this date:</h3>
          <div className="flex flex-wrap gap-2 mt-2">
            {workout.availableSlots.slice(0, 5).map((time) => {
              ...
              const timeRange =
                startAmPm === endAmPm
                  ? `${startHour12}:${minutes.toString().padStart(2, "0")} - ${endHour12}:${minutes
                      .toString()
                      .padStart(2, "0")} ${startAmPm}`
                  : `${startHour12}:${minutes.toString().padStart(2, "0")} ${startAmPm} - ${endHour12}:${minutes
                      .toString()
              ...
              return (
                <span
                  ...
                  className={`px-3 py-1 border rounded-md text-sm ${
                    isDarkMode ? "border-gray-600 bg-gray-700 text-white" : "border-gray-200 bg-lime-100 text-gray-600"
                  }`}
                >
                  {timeRange}
                </span>
        ...
        <div className="mt-4 flex flex-row gap-2">
          <Link
            to={`/coach-profile/${workout.coachId}`}
            className={`w-1/2 px-8 py-3 border flex justify-center rounded-lg font-semibold transition cursor-pointer ${
              isDarkMode
          ...
          <button
            className={`w-1/2 px-8 py-3 text-base font-semibold rounded-lg transition cursor-pointer ${
              isDarkMode
                ? "bg-lime-500 text-black hover:bg-lime-400"
                : "bg-[#9EF300] text-black hover:bg-lime-500"
            }`}
